...
import { ApigeeTemplateService, ApigeeTemplater } from 'apigee-templater-module'
...
describe('Generate simple normal JSON 1 proxy with customized api key plugin', () => {
  return it('should produce a valid proxy bundle', () => {
    ...
    return apigeeTemplater.generateProxyFromString(input, 'test/proxies').then((response) => {
      ...
      return fs.createReadStream(response.localPath).pipe(Extract({ path: "test/proxies/" + response.template?.name })).promise().then((result) => {
        ...
        expect(policyFileText).to.contain('<APIKey ref="request.header.x-api-key"/>');
